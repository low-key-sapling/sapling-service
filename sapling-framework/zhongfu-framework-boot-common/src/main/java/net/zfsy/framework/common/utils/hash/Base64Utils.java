package net.zfsy.framework.common.utils.hash;
...
public class Base64Utils {
    ...
    public static String encoder(String encoderStr, String charset) {
        try {
            if (StringUtils.isEmpty(encoderStr)) {
                throw new BasicException(AppHttpStatus.NULL_POINTER.getStatus(), "编码字符串不可以为空");
            }
    ...
    public static String decoder(String decoderStr, String charset) {
        try {
            if (StringUtils.isEmpty(decoderStr)) {
                throw new BasicException(AppHttpStatus.NULL_POINTER.getStatus(), "解码字符串不可以为空");
            }
            if (StringUtils.isEmpty(charset)) {
                charset = CharsetInfo.UTF_8;
            }
    ...
    public static void encoderWrap(String filePath, String encoderStr, String charset) {
        try {
            ...
            throw new BasicException(AppHttpStatus.ILLEGAL_PARAMETER.getStatus(), StringUtils.join("指定路径", filePath, "的文件不存在," + e));
        } catch (UnsupportedEncodingException e) {
            throw new BasicException(AppHttpStatus.ILLEGAL_PARAMETER);
        }
    }
